...
/// IGNORED (bead rust-oracledb-n2s): the INVARIANT the wide-row fix must satisfy
/// — an interior `0x1d` byte at a packet boundary must NOT end the response.
///
/// We frame a multi-packet response where an INTERIOR DATA packet's post-flags
/// payload ends in an ordinary `0x1d` byte — exactly what happens past ~1500
/// rows when a packet boundary lands on a normal row-data byte equal to
...
///
/// This test asserts the IDEAL: the reassembler reassembles the WHOLE response
/// (it does not stop on the interior 0x1d). The reference-correct rule satisfies
/// this; the NAIVE rule — which mirrors the UNFIXED driver reassembler
/// (`data_packet_ends_response` before bead n2s lands its `packet_size ==
/// header + 3` size guard) — VIOLATES it by truncating at the interior packet.
///
/// It is `#[ignore]`d because the naive rule it exercises is the bug: the test
/// FAILS under the naive rule today and FLIPS TO PASSING once the size-guarded
/// rule is the only reassembler. The passing split-invariance tests above
/// already use the correct rule and are the green guard the n2s fix must keep
/// satisfied. Run with:
///   cargo test -p oracledb-protocol --test framing_harness -- --ignored
#[test]
#[ignore = "bead rust-oracledb-n2s: fails under the unfixed (naive) reassembler; \
            flips green once the size-guard rule lands"]
fn interior_0x1d_must_not_end_the_response() {
    ...
    // INVARIANT: the reassembler must consume the whole response. The naive rule
    // (unfixed driver) truncates at the first interior packet, so this assertion
    // fails today — hence #[ignore]. Swap to `has_end_of_response_correct` (or
    // land bead n2s) and it passes.
    ...
    assert_eq!(
        reassembled.len(),
        full_len,
        "interior 0x1d at a packet boundary must NOT end the response \
         (bead rust-oracledb-n2s): got {} of {} bytes — the reassembler \
         stopped early",
        reassembled.len(),
        full_len
    );
