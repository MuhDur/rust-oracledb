...
| Low-level raw **execute**: `execute_raw(sql, prefetch, bind_rows, ExecuteOptions, timeout_ms) -> QueryResult` (async `Connection` + blocking `BlockingConnection`) | **Retained** (W2-T1, decision A). The execute-side counterpart to the fetch primitives above: the four families project the raw `QueryResult` into curated `Rows`/`ExecuteOutcome`/`BatchOutcome`, so a wire-faithful consumer (statement-type-agnostic dispatch on `result.columns`, parse-only describe, per-bind-row OUT/RETURNING aggregation) needs an un-deprecated raw entry point. This gap was surfaced by the 0.3.0 pyshim migration — W1-T3 shipped the fetch primitives but missed the symmetric raw execute. It is **not** a re-export of the deprecated `execute_query*` sprawl: it is the single minimal raw primitive those consumers genuinely need; the deprecated names stay deprecated and removable at W4-T1. |
