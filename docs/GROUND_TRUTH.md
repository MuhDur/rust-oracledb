...
| `crates/oracledb` | 1,640 (single `lib.rs`) | async driver: `Connection` with `&Cx`-first `pub async fn` API (connect/ping/commit/rollback/execute/fetch/LOB ops/cancel/close) on **asupersync 0.3.9** (no tokio), plus a `BlockingConnection` facade (lib.rs:942-1253) |
