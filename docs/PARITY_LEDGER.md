...
| #596 | TSTZ column converted incorrectly to Arrow (thin, fixed upstream 714178610379 to emit wall-clock tz-naive) | **Reconciled — now matches upstream** | Our 0.5.1 Arrow path UTC-normalized TSTZ (subtracted the offset), which was a divergence we introduced — NOT immunity. As of 0.8.3 the Arrow path emits the **wall-clock** tz-naive value (offset dropped), matching upstream's #596 fix and Arrow's tz-naive semantics (a `Timestamp(_, None)` field carries no zone). LTZ already decoded as offset-less `DateTime`, so it was always wall-clock. The offset-preserving instant remains available on the row (non-Arrow) fetch path. Guard: `arrow/mod.rs:timestamp_tz_maps_to_arrow_wall_clock` + `timestamp_tz_arrow_wall_clock_independent_of_offset`. |
