...
| Trust anchors | **Union of system and wallet roots.** System roots are loaded through `rustls-native-certs` 0.8.4 from the platform store; `SSL_CERT_FILE` / `SSL_CERT_DIR` select explicit system-root locations instead. When a wallet is present, its CA certificates are added to those roots, so a wallet does **not** narrow platform trust. Legacy Unix bundle paths are only a no-root fallback when neither override is set. Zero usable anchors → `Error::Tls("no trust anchors…")` (**fail closed**). | union `crates/oracledb/src/tls.rs` (`build_client_config`, `union_trust_anchors`); system loader and override guard `load_system_roots`, `explicit_root_override_is_set`; dependency `Cargo.toml:31-34` |
...
| DSN `SECURITY` wallet directory + DN-match/cert-DN (**F1** / **F-DC2** / **F-DC3**) | **Applied.** `SSL_SERVER_CERT_DN` and the DSN wallet directory (`MY_WALLET_DIRECTORY` / `WALLET_LOCATION`) feed the TLS setup with **options-over-DSN** precedence (explicit `ConnectOptions` wins, else the DSN `SECURITY` value). **`SSL_SERVER_DN_MATCH` is fail-closed AND-merge**: effective match = DSN `SECURITY.SSL_SERVER_DN_MATCH` **and** `ConnectOptions::ssl_server_dn_match` (both default `true`). DSN-only `OFF` therefore disables identity matching even when the builder is left at its default; an explicit `with_ssl_server_dn_match(false)` also disables it when the DSN says `ON`. OFF disables only the Oracle DN/SAN/CN identity check — **TLS chain-of-trust validation still applies** (`OracleServerCertVerifier` still verifies the leaf to a trust anchor). Offline-proven by F-DC2/F-DC3 discriminating tests in `crates/oracledb/src/lib.rs` (`f_dc2_*`, `f_dc3_*`, landed `be26a50`); a full live-ADB TCPS acceptance still needs ADB infra (**intended/unverified**, see §2). | `resolve_tls_params` / `Connection::connect`, `crates/oracledb/src/{lib,tls}.rs` |
| Oracle Server Name Indication (SNI) over TCPS (**F3** / P2-4) | **Fail closed** when requested-but-un-encodable, with an **OCI ADB host-as-SNI carve-out**. `use_sni=true` builds the Oracle service-form token (`S{len}.{service}.V3.{version}`); rustls cannot carry it (RFC-strict DNS). Non-ADB: typed `Error::UnsupportedSni`. OCI ADB shared/PE/sovereign hosts: send the listener host as SNI so the handshake completes (routing fast-path forgone — see `PARITY_LEDGER.md` `sni-service-form`). DN match remains authoritative. Default remains `use_sni=false`. Offline-proven (`decide_sni` unit tests). | `decide_sni` / `is_oci_adb_endpoint`, `crates/oracledb/src/tls.rs` |
