...
| `TIMESTAMP WITH TIME ZONE` — fixed UTC offset | Supported; the hour/minute offset bytes are decoded against `TZ_HOUR_OFFSET`/`TZ_MINUTE_OFFSET` markers. | `crates/oracledb-protocol/src/thin/codecs.rs:99-109`; markers `constants.rs:341-342` |
